...
- **Oscillation verticale** : `FilmT.animBob?: Record<animationId, { amplitudePx, phase? }>` — le sampler ajoute à `y` une sinusoïde par CYCLE de l'animation (`animFrame / frameCount`, × échelle du perso) partout où elle joue (idle, trajet, clip) → vol qui « pompe » avec les ailes. Réglé dans la carte « Oscillation verticale » de `FilmEditorT`. Persisté `filmT.animBob`.
