...
- **Modèle** (`types/project.ts`) : `FilmT { version: 4, plans: FilmTimelinePlan[], character, sounds (bibliothèque), music?, intro?/outro? (ouverture/fermeture du film, même modèle FilmPlanTransition — rendues par `startFilmEdgeTransition` reveal/conceal en aplat couleur, réglées dans la barre des plans ; legacy introFadeMs/outroFadeMs lus en fallback via `filmIntroTransition`/`filmOutroTransition`), défauts éditeur }`. Chaque plan : décor (`backdrop`/`overlay` chroma), `cameraX`, `transitionToNext` (kind + `durationMs` + `color` : fondu couleur / volet coloré — sans `color` le volet glisse le snapshot), et SA `timeline { durationMs, waypoints[] (spatial : x/y/échelle/regard), motion[] (1 piste exclusive : appear/travel/exit, durée maîtresse → vitesse dérivée, courbes Bézier + easing, 🔒 lockedSpeedPxPerSec), anim[] (1 piste : animationId + fillMode loop/once-hold), soundTracks[][] (N pistes libres : volume/rate/loop/parlé/fades + ancrage ⚓ {clipId, edge, offsetMs} sur un clip motion/anim) }`. Les types v3 (`Film`/`FilmPlan` = alias `FilmV3`/`FilmPlanV3`) ne servent qu'à la lecture legacy.
...
- **Pistes globales** : `FilmT.globalSoundTracks?: FilmSoundClip[][]` — clips son en temps FILM ABSOLU (intro comprise), à cheval sur les plans (poursuite, nappe…), sans ancrage ⚓. Éditées via le bouton **🌐 Timeline globale** de `FilmEditorT` (plans/transitions/musique figés en rangées lecture seule + pistes 🌐, `TimelineEditor` en `globalMode`, lecture AUDIO du film entier avec Espace). Le scheduler les aplatit à base 0 ; en lecture d'un seul plan, `opts.globalOffsetMs` = début absolu du plan. Les clips son portent aussi `offsetMs` (rognage du début du fichier par la poignée gauche). Persistance `ProjectDoc.filmT.globalSoundTracks` (wrappé `{clips}`).
- **Oscillation verticale** : effet de la PISTE CAMÉRA `kind: 'bob'` (`FilmCameraClip` : `amplitude` px décor, `frequencyHz`, `phase` 0..1) — la caméra monte/descend en sinusoïde régulière (`filmCamera.evaluateCamera`, additif sur `shakeY`). Bouton « Oscillation » de la barre caméra : clip ancré ⚓ à l'animation en cours, fréquence = 1 oscillation par cycle VISUEL de l'animation (`FILM_FPS × vitesse ÷ (frames − crossfade)`) ; bouton « ⟲ Caler sur… » dans l'inspecteur pour recalculer. Pas de réglage global par animation.
- **Horloge unique (2026-09-23)** : en mode film, les `LoopPlayback` ne sont plus avancés au ticker PIXI mais **posés chaque tick sur la phase du sampler** (`seekFrame(animFrame mod période)`) — zéro dérive maillage/son, y compris pendant une transition où le temps local du plan entrant est figé à 0. Le plan entrant est évalué à son temps ABSOLU (`planStartMs`), plus de recul de temps en fin de transition. L'AudioContext a UN seul pilote sérialisé (`setSharedAudioContextRunning`, `mouthAudioAnalyser.ts`) qui gère l'interruption iOS ; le film n'appelle `scheduler.start()` qu'après `unlock()` (1,5 s max) ; si l'horloge du ctx ne bouge plus 500 ms, bascule sur l'horloge murale (film muet mais vivant). Bruits de pas : un seul planning par intervalle (`footstepSync`), dédoublonnage < 60 ms. Sonde `window.__filmDebug` (tMs, ctxMs, planIndex, animFrame, meshFrame…) pour le diagnostic / banc headless.
...
- **Bruits de pas** : réglés au niveau de l'ANIMATION de marche (étape « Bruits de pas » en fin de pipeline marche, `MarcheFootstepsStep`) — frames de contact VALIDÉES à la main sur une bande de frames (`mesh.footstepFrames`, pré-remplies par `detectFootstepFrames`), sons `Animation.footstepSound1/2Blob` alternés (Storage `animations/{animId}/footstep1|footstep2`), volume/décalage sur le mesh. Le film ne porte qu'une case `FilmT.footstepsEnabled` (défaut true) et un gain `FilmT.footstepsVolume` (0–3, défaut 1, multiplie le volume de l'animation ; slider dans les réglages du film) ; un one-shot joue 450 ms max pour la marche, 1,5 s pour les sons de cycle d'un idle (`FootstepOneShot.maxMs`) ; `computeFootstepSchedule(film, animations, planStartMs)` pose les one-shots (clés `anim:{animId}:1|2` résolues via le paramètre `extraSounds` du `FilmAudioScheduler`). Sans frames validées : fallback heuristique UNIQUEMENT si l'animation a des sons de pas attachés. Le même modèle sert aux **sons de cycle** d'un idle en boucle (battement d'ailes) : étape « Sons du cycle » du pipeline cotracker-bones (même composant `MarcheFootstepsStep`, libellés génériques, détection point bas / point haut via `detectFootstepFrames(anim, zones, mode)`). Une animation sans son attaché ne produit rien (anciens projets inchangés).
...
- **Parcours play (2026-09-24)** : UN bouton SCAN → caméra directe, reconnaissance automatique du QR (plus d'écran « Ajouter un livre / Scanner un coloriage », plus de confirmation). QR livre → ajout + téléchargement en arrière-plan + retour au menu ; QR coloriage → le livre qui le contient s'ajoute tout seul (`project.bookId`) puis `/p/:id?autocam=1`. Accueil sans livre : « Bienvenue ! Scanne ton premier coloriage » + flèche animée vers SCAN (plus de carte « + »). Livre en téléchargement : anneau de % (3 étapes par coloriage, `bookAddedAt` puis `bookDownloadedAt`, reprise automatique à l'accueil si l'app a été fermée), carte non ouvrable jusqu'à 100 % puis animation « prêt ». Sample gratuit = UN livre « FREE DINOSAUR COLORING » avec un seul coloriage.
- **Robustesse play** : `apps/play/src/components/ErrorBoundary.tsx` (écran « Réessayer » au lieu d'un écran blanc) + `apps/play/src/utils/appErrors.ts` (20 dernières erreurs — rendu React, exceptions, promesses rejetées — dans `localStorage['picopop.errors']`, lisible via Web Inspector). Réseau : « connexion lente » = 5 s sans octet reçu (`useDownloadProgress`), suivi démarré avant la requête (`blobCache`), préchargement des livres compté en arrière-plan (`runAsBackgroundDownloads`) et mis en pause pendant un scan. Film : résolution PIXI plafonnée à 2×, boutons HUD armés après 700 ms, décodage audio plafonné 3 s / retenue d'ouverture 4 s, pas d'artefacts de debug LaMa en play.
