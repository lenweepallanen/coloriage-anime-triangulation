...
| `MembersBonesV2LegBoneStep.tsx` | 13 "Bones Pattes" | Éditeur leg bones uniquement (body chain read-only). Supporte hip par anchor OR par body vertex unique (`hipBodyVertexIndex`). Preview utilise `walkBodyFramesSmoothed ?? walkBodyFrames`. **Toggle Image (haute res, défaut on) / Video** en background — l'image projet est rendue en bypass du transform canvas pour rester nette même fortement zoomée. Bouton "Sauvegarder" persiste les pattes une par une (sans valider) ; "Valider" requiert toutes pattes complètes. Indicateur "✓ Sauvegardé à HH:MM:SS" après save réussi. |
...
| `MembersBonesV3BodyComputeStep.tsx` | 7 "Calc Body ARAP" | `precomputeARAP(bodyPoints, bodyTriangles, pinnedContourIndices)` depuis `projectTriangulation`, solve par frame. Produit `walkBodyFrames` (en coords IMAGE). Boundary frame 0 vient de `tri.bodyPoints.slice(0, nContour)`. Per-frame boundary via `buildV3BoundaryFrames(mesh, imgW, imgH, subParams)` — ordre interleavé `[P0, ...subs_seg0, anchor_1, ...subs_seg1, ..., anchor_N, ...subs_segN]` matchant `tri.bodyPoints[0..nContour]`. |
