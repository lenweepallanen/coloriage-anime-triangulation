...
export interface CSSSnapResult {
  position: Point2D
...
// ─── Détection multi-échelle + NMS ──────────────────────────────────

/**
 * Détecte les points caractéristiques CSS sur un contour ordonné.
 *
 * Algorithme :
 * 1. Rééchantillonne à N points uniformes
 * 2. Pour chaque σ : calcule κ → trouve les extrema
 * 3. Collecte les candidats depuis TOUTES les échelles (pas seulement σ_max)
 * 4. Score = persistance multi-échelle × prominence
 * 5. NMS en arc-length circulaire
 */
export function detectCSSCandidates(
  path: Point2D[],
  scales: number[] = [1, 2, 4, 8, 16],
  maxCandidates: number = 8,
  minArcSpacing: number = 1 / maxCandidates
): CSSCandidate[] {
  ...
  const sigmaMax = Math.max(...scales)

  // Déterminer l'orientation du contour (CW/CCW) pour classifier convexe/concave
  const orientation = contourOrientation(xs, ys)

  // Collecter les peaks à chaque échelle
  const peaksByScale: Map<number, CurvaturePeak[]> = new Map()
  for (const sigma of scales) {
    const kappa = computeCurvature(xs, ys, sigma)
    const peaks = findCurvatureExtrema(kappa, 0.001, orientation)
    peaksByScale.set(sigma, peaks)
  }

  // Collecter les candidats depuis TOUTES les échelles (pas seulement σ_max)
  // Un candidat = une position arc-length unique trouvée à au moins une échelle
  // Score = nombre d'échelles où le pic persiste × somme pondérée des prominences
  const rawCandidates: { arcLengthNorm: number; index: number; scaleHits: Map<number, CurvaturePeak> }[] = []

  // Commencer par les pics aux échelles les plus grandes (features les plus stables)
  const sortedScales = [...scales].sort((a, b) => b - a)

  for (const sigma of sortedScales) {
    const scalePeaks = peaksByScale.get(sigma) ?? []
    for (const peak of scalePeaks) {
      // Chercher si un candidat existant est proche
      const matchRadius = Math.max(0.02, (3 * sigma) / N)  // fenêtre de correspondance
      let merged = false

      for (const existing of rawCandidates) {
        let d = Math.abs(existing.arcLengthNorm - peak.arcLengthNorm)
        if (d > 0.5) d = 1 - d
        if (d < matchRadius) {
          existing.scaleHits.set(sigma, peak)
          merged = true
          break
        }
      }
      ...
      if (!merged) {
        const hits = new Map<number, CurvaturePeak>()
        hits.set(sigma, peak)
        rawCandidates.push({
  ...
  // score = Σ (σ/σ_max × prominence) × persistance × biais convexité
  const scored: CSSCandidate[] = []

  for (const candidate of rawCandidates) {
    let score = 0
    for (const [sigma, peak] of candidate.scaleHits) {
      score += (sigma / sigmaMax) * peak.prominence
    }
    // Bonus de persistance : un pic visible à 4/5 échelles est plus fiable qu'un pic à 1 seule
    const persistence = candidate.scaleHits.size / scales.length
    score *= (0.5 + 0.5 * persistence)

    // Biais de convexité : les points convexes (sommets, bouts) sont privilégiés
    // par rapport aux concavités (échancrures, plis internes)
    // Déterminé par le pic à l'échelle la plus grande disponible
    let bestSigma = 0
    ...
    for (const [sigma, peak] of candidate.scaleHits) {
      if (sigma > bestSigma) {
        bestSigma = sigma
        bestIndex = peak.index
        isConvex = peak.isConvex
      }
    }

    // Convexe ×2, concave ×0.3
    score *= isConvex ? 2.0 : 0.3

    scored.push({
      arcLengthNorm: bestIndex / N,
      position: points[bestIndex],
      score,
    })
  }
  ...
  for (const candidate of scored) {
    ...
    for (const acc of accepted) {
      let d = Math.abs(candidate.arcLengthNorm - acc.arcLengthNorm)
      if (d > 0.5) d = 1 - d
      if (d < minArcSpacing) {
        tooClose = true
        break
...
export interface ExtremaTrackingResult {
  /** Extrema détectés à cette frame, réordonnés selon le matching */
  extrema: CSSCandidate[]
  /** matching[i] = index dans les extrema bruts correspondant au i-ème extremum précédent. -1 si perdu */
  matching: number[]
}

/**
 * Détecte les top N extrema de courbure sur un contour, puis les matche
 * aux extrema de la frame précédente par assignment glouton (proximité spatiale).
 *
 * À la frame 0, passer previousExtrema = null → retourne simplement les extrema détectés.
 */
export function trackCurvatureExtrema(
  orderedContour: Point2D[],
  numExtrema: number,
  previousExtrema: CSSCandidate[] | null,
  sigma: number = 4
): ExtremaTrackingResult {
  const currentExtrema = detectCurvatureExtrema(orderedContour, numExtrema, sigma)

  // Frame 0 ou pas de précédent → pas de matching
  if (!previousExtrema || previousExtrema.length === 0) {
    return {
      extrema: currentExtrema,
      matching: currentExtrema.map((_, i) => i),
    }
  }

  // Assignment glouton : trier toutes les paires (prev_i, curr_j) par distance croissante
  const pairs: { prevIdx: number; currIdx: number; dist: number }[] = []
  for (let i = 0; i < previousExtrema.length; i++) {
    for (let j = 0; j < currentExtrema.length; j++) {
      const dx = previousExtrema[i].position.x - currentExtrema[j].position.x
      const dy = previousExtrema[i].position.y - currentExtrema[j].position.y
      pairs.push({ prevIdx: i, currIdx: j, dist: Math.sqrt(dx * dx + dy * dy) })
    }
  }
  pairs.sort((a, b) => a.dist - b.dist)

  // Assigner chaque prev à un curr (et vice-versa) une seule fois
  const assignedPrev = new Set<number>()
  const assignedCurr = new Set<number>()
  // matching[prevIdx] = currIdx ou -1 si non assigné
  const matching = new Array<number>(previousExtrema.length).fill(-1)

  for (const pair of pairs) {
    if (assignedPrev.has(pair.prevIdx) || assignedCurr.has(pair.currIdx)) continue
    matching[pair.prevIdx] = pair.currIdx
    assignedPrev.add(pair.prevIdx)
    assignedCurr.add(pair.currIdx)
    if (assignedPrev.size === previousExtrema.length) break
  }

  // Réordonner currentExtrema selon le matching :
  // reordered[i] = currentExtrema[matching[i]], ou previousExtrema[i] si perdu
  const reordered: CSSCandidate[] = matching.map((currIdx, prevIdx) =>
    currIdx >= 0 ? currentExtrema[currIdx] : previousExtrema[prevIdx]
  )

  return { extrema: reordered, matching }
}
...
// ─── Détection par signature intrinsèque (s, κ, κ') ─────────────────

export interface IntrinsicSignature {
  s: number          // arc-length normalisé [0,1)
  kappa: number      // courbure κ
  kappaPrime: number // dérivée dκ/ds
}

export interface IntrinsicMatchResult {
  positions: Point2D[]
  unresolvedFlags: boolean[]
}

/**
 * Calcule le champ de signature intrinsèque (s, κ, κ') sur tout le contour.
 * Retourne les points rééchantillonnés et leur signature dense.
 */
export function computeContourSignatureField(
  orderedPath: Point2D[],
  numSamples?: number,
  sigma: number = 16
): { points: Point2D[]; signatures: IntrinsicSignature[]; totalLength: number } {
  const N = numSamples ?? Math.min(orderedPath.length, 1024)
  const { points, totalLength } = reparameterizeByArcLength(orderedPath, N)

  if (totalLength < 1e-6) {
    return {
      points,
      signatures: points.map((_, i) => ({ s: i / N, kappa: 0, kappaPrime: 0 })),
      totalLength,
    }
  }
  ...
  // Dérivée de courbure par différences finies centrales (wrap-around cyclique)
  const ds = totalLength / N
  const signatures: IntrinsicSignature[] = new Array(N)
  for (let i = 0; i < N; i++) {
    const kNext = kappa[(i + 1) % N]
    const kPrev = kappa[(i - 1 + N) % N]
    signatures[i] = {
      s: i / N,
      kappa: kappa[i],
      kappaPrime: (kNext - kPrev) / (2 * ds),
    }
  }

  return { points, signatures, totalLength }
}

/**
 * Enregistre la signature intrinsèque de chaque anchor à la frame 0.
 * Pour chaque anchor, trouve le sample rééchantillonné le plus proche.
 */
export function computeAnchorIntrinsicSignatures(
  anchors: Point2D[],
  orderedPath: Point2D[],
  sigma: number = 16
): IntrinsicSignature[] {
  const field = computeContourSignatureField(orderedPath, undefined, sigma)

  return anchors.map(anchor => {
    let bestIdx = 0
    let bestDistSq = Infinity
    for (let i = 0; i < field.points.length; i++) {
      const dx = anchor.x - field.points[i].x
      const dy = anchor.y - field.points[i].y
      const dSq = dx * dx + dy * dy
      if (dSq < bestDistSq) {
        bestDistSq = dSq
        bestIdx = i
    ...
    return field.signatures[bestIdx]
  })
}

/**
 * Matche les anchors sur un contour keyframe par minimisation de coût intrinsèque.
 * C(i,j) = |s_j - s₀|² + λ₁|κ_j - κ₀|² + λ₂|κ'_j - κ'₀|²
 * Contraintes : fenêtre arc-length + signe κ' identique.
 */
export function matchAnchorsIntrinsic(
  referenceSignatures: IntrinsicSignature[],
  field: { points: Point2D[]; signatures: IntrinsicSignature[] },
  options?: {
    lambda1?: number
    lambda2?: number
    arcThreshold?: number
  }
): IntrinsicMatchResult {
  const lambda1 = options?.lambda1 ?? 1.0
  const lambda2 = options?.lambda2 ?? 1.0
  const arcThreshold = options?.arcThreshold ?? 0.10
  const KAPPA_PRIME_EPSILON = 0.001

  const N = field.signatures.length
  const positions: Point2D[] = []
  const unresolvedFlags: boolean[] = []

  for (const ref of referenceSignatures) {
    let bestCost = Infinity
    let bestIdx = -1

    for (let j = 0; j < N; j++) {
      const sig = field.signatures[j]

      // Contrainte 1 : fenêtre arc-length circulaire
      let arcDist = Math.abs(sig.s - ref.s)
      if (arcDist > 0.5) arcDist = 1 - arcDist
      if (arcDist >= arcThreshold) continue

      // Contrainte 2 : signe κ' identique (relaxé si κ'₀ ≈ 0)
      if (Math.abs(ref.kappaPrime) >= KAPPA_PRIME_EPSILON) {
        if (Math.sign(sig.kappaPrime) !== Math.sign(ref.kappaPrime)) continue
      }

      // Coût
      const cost = arcDist * arcDist
        + lambda1 * (sig.kappa - ref.kappa) * (sig.kappa - ref.kappa)
        + lambda2 * (sig.kappaPrime - ref.kappaPrime) * (sig.kappaPrime - ref.kappaPrime)
    ...
    if (bestIdx >= 0) {
      positions.push({ ...field.points[bestIdx] })
      unresolvedFlags.push(false)
    } else {
      // Non résolu — fallback : position à s₀ sur le contour
      const fallbackIdx = Math.round(ref.s * N) % N
      positions.push({ ...field.points[fallbackIdx] })
      unresolvedFlags.push(true)
    }
  }

  return { positions, unresolvedFlags }
}
